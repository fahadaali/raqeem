...
export async function validateRows(app, ctx, type, rows) {
  ...
  const valid = [];
  const seen = new Set();
  const branches = (await app.db.all('SELECT code FROM branches WHERE tenant_id=?', ctx.tenantId)).map(b => b.code.toLowerCase());
  const roles = (await app.db.all('SELECT key FROM roles WHERE tenant_id=?', ctx.tenantId)).map(r => r.key);
  const existingEmails = new Set((await app.db.all('SELECT lower(email) AS e FROM users WHERE tenant_id=?', ctx.tenantId)).map(u => u.e));
...
export async function runImport(app, { batchId, tenantId, userId, type, rows, branchId }) {
  await app.db.run(`UPDATE import_batches SET status='running' WHERE id=?`, batchId);
  let ok = 0;
  const errors = [];

  const branchByCode = Object.fromEntries(
    (await app.db.all('SELECT id, lower(code) AS c FROM branches WHERE tenant_id=?', tenantId)).map(b => [b.c, b.id]));
  const roleByKey = Object.fromEntries(
    (await app.db.all('SELECT id, key FROM roles WHERE tenant_id=?', tenantId)).map(r => [r.key, r.id]));
  const term = await app.db.get('SELECT id FROM terms WHERE tenant_id=? AND is_current=1', tenantId);

  const emailIndex = async () => Object.fromEntries(
    (await app.db.all('SELECT id, lower(email) AS e FROM users WHERE tenant_id=?', tenantId)).map(u => [u.e, u.id]));

  const CHUNK = 50;
  for (let i = 0; i < rows.length; i += CHUNK) {
    const slice = rows.slice(i, i + CHUNK);
    const emails = await emailIndex();
    const stmts = [];
    const rowRefs = [];
        ...
        if (type === 'users') {
          ...
          stmts.push([`INSERT INTO users(tenant_id,name,email,phone,password_hash,role_id,primary_branch_id,must_change_pw)
            ...
            [tenantId, r.name, r.email.toLowerCase(), r.phone || null, await hashPassword(pw),
             roleByKey[r.role_key], branchByCode[r.branch_code.toLowerCase()]]]);
          rowRefs.push({ r, kind: 'user', branch: branchByCode[r.branch_code.toLowerCase()] });
        } else if (type === 'employees') {
          const uid = emails[r.email.toLowerCase()];
          stmts.push([`INSERT INTO employees(tenant_id,branch_id,user_id,employee_no,job_title,department,hire_date,basic_salary,allowances,bank_iban)
            ...
            [tenantId, uid, uid, r.employee_no || null, r.job_title || null, r.department || null,
             r.hire_date || null, num(r.basic_salary), num(r.allowances), r.bank_iban || null]]);
          rowRefs.push({ r, kind: 'other' });
        } else if (type === 'branches') {
          stmts.push([`INSERT INTO branches(tenant_id,code,name,address,lat,lng,geofence_radius,phone) VALUES(?,?,?,?,?,?,?,?)`,
            [tenantId, r.code, r.name, r.address || null, r.lat ? num(r.lat) : null,
             r.lng ? num(r.lng) : null, num(r.geofence_radius) || 50, r.phone || null]]);
          rowRefs.push({ r, kind: 'other' });
        } else if (type === 'tasks') {
          const bid = r.branch_code ? branchByCode[r.branch_code.toLowerCase()] : branchId;
          const com = r.committee_name
            ? (await app.db.get('SELECT id FROM committees WHERE tenant_id=? AND name=?', tenantId, r.committee_name))?.id
            : null;
          stmts.push([`INSERT INTO tasks(tenant_id,branch_id,term_id,committee_id,title,description,priority,assignee_id,created_by,start_date,due_date)
            VALUES(?,?,?,?,?,?,?,?,?,?,?)`,
            [tenantId, bid || null, term?.id || null, com || null, r.title, r.description || null,
             r.priority || 'medium', r.assignee_email ? emails[r.assignee_email.toLowerCase()] || null : null,
             userId, r.start_date || null, r.due_date || null]]);
          rowRefs.push({ r, kind: 'other' });
        } else if (type === 'budgets') {
          stmts.push(['INSERT INTO budgets(tenant_id,branch_id,term_id,name,category,amount) VALUES(?,?,?,?,?,?)',
            [tenantId, r.branch_code ? branchByCode[r.branch_code.toLowerCase()] : branchId,
             term?.id || null, r.name, r.category || 'عام', num(r.amount)]]);
          rowRefs.push({ r, kind: 'other' });
        } else if (type === 'advances') {
          const uid = emails[r.email.toLowerCase()];
          stmts.push([`INSERT INTO advances(tenant_id,branch_id,term_id,user_id,amount,remaining,installments,reason,date,status)
            VALUES(?,?,?,?,?,?,?,?,?,'approved')`,
            [tenantId, branchId, term?.id || null, uid, num(r.amount), num(r.amount),
             num(r.installments) || 1, r.reason || null, r.date || new Date().toISOString().slice(0, 10)]]);
          rowRefs.push({ r, kind: 'other' });
        }
      } catch (e) {
    ...
    try {
      ...
      ok += stmts.length;
      // تسكين المستخدمين المستوردين في فروعهم
      const links = [];
      res.forEach((out, k) => {
        const ref = rowRefs[k];
        if (ref?.kind === 'user' && ref.branch && out.lastId) {
          links.push(['INSERT OR IGNORE INTO user_branches(tenant_id,user_id,branch_id) VALUES(?,?,?)',
            [tenantId, out.lastId, ref.branch]]);
        }
      });
      if (links.length) await app.db.batch(links);
    } catch (e) {
      // عند فشل الدفعة نعيد المحاولة صفاً صفاً لعزل السجل المخالف
      for (let k = 0; k < stmts.length; k++) {
        try {
          const out = await app.db.run(stmts[k][0], ...stmts[k][1]);
          ok++;
          const ref = rowRefs[k];
          if (ref?.kind === 'user' && ref.branch && out.lastId) {
            await app.db.run('INSERT OR IGNORE INTO user_branches(tenant_id,user_id,branch_id) VALUES(?,?,?)',
              tenantId, out.lastId, ref.branch);
          }
        } catch (err) {
          errors.push({ row: rowRefs[k]?.r?._row ?? '-', field: '-', message: String(err.message).slice(0, 200) });
        }
      }
    }
  }

  const prev = await app.db.get('SELECT errors, error_rows FROM import_batches WHERE id=?', batchId);
  const allErrors = [...JSON.parse(prev.errors || '[]'), ...errors].slice(0, 300);
  await app.db.run(`UPDATE import_batches SET status=?, ok_rows=?, error_rows=?, errors=?, finished_at=? WHERE id=?`,
    errors.length && !ok ? 'failed' : 'done', ok, prev.error_rows + errors.length,
    JSON.stringify(allErrors), nowUTC(), batchId);

  await auditSystem(app, { tenantId, action: 'create', entity: 'import_batch', entityId: batchId,
