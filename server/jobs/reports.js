...
import { scoped, currentTerm } from '../lib/scope.js';
...
export const REPORTS = {
  ...
  budgets: {
    ...
    run(ctx, f) {
      ...
      const raw = db.prepare(sql + ' ORDER BY b.name').all(...p);
      return {
        columns: [
  ...
  payroll: {
    ...
    run(ctx, f) {
      ...
      const raw = db.prepare(sql + ' ORDER BY u.name').all(...p);
      return {
        columns: [
  ...
  kpi: {
    ...
    run(ctx, f) {
      ...
      const raw = db.prepare(sql + ' ORDER BY k.score DESC').all(...p);
      return {
        columns: [
  ...
  tickets: {
    ...
    run(ctx, f) {
      ...
      const raw = db.prepare(sql + ' ORDER BY t.created_at DESC').all(...p);
  ...
  audit: {
    ...
    run(ctx, f) {
      ...
      const raw = db.prepare(sql + ' ORDER BY id DESC LIMIT 5000').all(...p);
...
export function runReport(ctx, key, filters = {}) {
  ...
  if (!clean.term_id && def.filters.some(f => f.key === 'term_id')) {
    const t = currentTerm(ctx.tenantId); if (t) clean.term_id = t.id;
  }
  const out = def.run(ctx, clean);

  const labelOf = (k, v) => {
    if (k === 'branch_id') return db.prepare('SELECT name FROM branches WHERE id=?').get(v)?.name || v;
    if (k === 'term_id') return db.prepare('SELECT name FROM terms WHERE id=?').get(v)?.name || v;
    if (k === 'user_id') return db.prepare('SELECT name FROM users WHERE id=?').get(v)?.name || v;
    const fd = def.filters.find(f => f.key === k);
    return fd?.options?.find(o => String(o.value) === String(v))?.label || v;
  };
  return {
    ...out,
    applied_filters: Object.entries(clean).map(([k, v]) => ({
      label: def.filters.find(f => f.key === k)?.label || k, value: labelOf(k, v)
    }))
  };
}
